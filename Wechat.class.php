...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function createCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getPrepayId($openid, $body, $out_trade_no, $total_fee, $notify_url, $trade_type = "JSAPI") {
        ...
        $result = $this->http_post(self::MCH_BASE_URL . '/pay/unifiedorder', self::xml_encode($postdata));
        if ($result) {
            $json = (array) simplexml_load_string($result, 'SimpleXMLElement', LIBXML_NOCDATA);
            if (!$json || $json['return_code'] !== 'SUCCESS') {
     ...
     * @return boolean
     */
    public function getNative($body, $out_trade_no, $total_fee, $notify_url, $trade_type = "NATIVE") {
        if (!$this->access_token && !$this->checkAuth()) {
            return false;
        }
        $postdata = array(
